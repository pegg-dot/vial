...
- `VIAL_SESSION_SECRET` — ≥32 chars, random
- `VIAL_PRIVACY_HASH_SECRET` — ≥32 chars, random
- `NEXT_PUBLIC_SITE_URL` — the public https URL (e.g. `https://vial.app`)
- `NODE_ENV=production`

Do NOT set (keep demos off):
- `VIAL_SEED_FIXTURES` — leave unset/`false`. Setting `true` would seed demo fixtures.
- `VIAL_SEED_DEMO_ACCOUNTS` — leave unset. Setting `true` would create the demo login accounts.
...
- `VIAL_LIVE_INGEST_APPROVED=true` — required to run the data bootstrap / collectors.
