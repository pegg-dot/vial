# VIAL — Complete Marketing Context Briefing

> **Purpose.** This is a full, self-contained briefing on VIAL for a marketer and their AI assistant.
...
> messaging: when unsure, under-claim.** VIAL's entire reason to exist is honesty about evidence, so
...
claim "99% pure," borrow someone else's lab certificate, or launch a scam site overnight. VIAL is a
...
the vendor to buy. **VIAL never sells anything, never touches money, and never tells you to use a
...
- **VIAL's thesis:** replace the vendor's *word* with the *receipt* — public, vendor-immutable
  evidence (lab certificates, government records, gathered reputation) presented decomposably so a
  non-expert can act on it.

## 3. What VIAL is — and explicitly is NOT
...
**The chain of evidence.** VIAL's homepage thesis is *"Trust is a chain of evidence."* Every public
...
Analysis) can still dodge most of what matters. On every listing VIAL breaks a test claim into
...
**B) COA cross-check (counterfeit detection).** For each listing, VIAL cross-checks the vendor's cited
...
- **Mismatch / borrowed certificate** — the cited certificate actually belongs to a **different
  manufacturer**. This is the clearest counterfeit signal, and VIAL now catches it consistently on
  the market grid, the product page, and the comparison tool.
- **No claim** — nothing advertised or on record.

**C) The composed vendor trust verdict — decomposable, never a black-box score.** Every seam VIAL
...
and the verdict shows *exactly which facts it rests on*. The seams:
  ...
  4. **Third-party trackers** — outside aggregator scores, shown as *their* opinion, not VIAL's.
...
sticker price lies. VIAL computes cost-per-mg and then **cost per active mg** (adjusted for measured
...
- **Product page (`/products/[slug]`)** — the buy box (price, market median, real cost/active mg,
  ...
  handoff ("Buy at vendor" → links to the vendor's own page; VIAL takes $0).
...
- **Testing (`/testing`)** — how the 279 certificates were **sampled**: blind purchase (strongest —
  the seller couldn't hand-pick the unit) vs vendor-selected. VIAL labels which is which rather than
...
- **Enforcement (`/enforcement`)** — the public regulatory-action record VIAL tracks.
...
- **Affiliate-out, click-tracked.** When a buyer clicks "Buy at vendor," VIAL records the click and
  ...
  after launch.
- **$0 from users.** VIAL never charges the buyer and never takes payment. ("what we take: $0" is
...
- **vs. vendor sites / testimonials:** conflicted; VIAL is independent and shows the receipts.
- **vs. affiliate "best peptide vendor" blogs:** pay-to-rank; VIAL ranks on **verifiable facts only,
  never who a product is "for."**
- **vs. Reddit / forums:** unstructured and gameable; VIAL structures the same community signal and
  cross-checks it against lab and government records.
- **vs. a lab:** VIAL doesn't test; it aggregates independent tests and, crucially, tells you **how
...
- ❌ Never position VIAL as **selling** anything or as a pharmacy/marketplace — it's a verification
...
- **Trust verdict:** VIAL's decomposable Trusted/Caution/Unproven/Avoid summary of all vendor signals.
