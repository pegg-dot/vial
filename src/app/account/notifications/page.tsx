...
export default async function Page(){const principal=await requirePrincipal({accountTypes:["customer","seller"]});await syncWatchlistNotifications(principal.id);const rows=await listUserNotifications(principal.id,{limit:100});const vapid=getVapidConfig();return <main className="mx-auto max-w-5xl px-5 py-14"><p className="text-[11px] font-semibold uppercase tracking-[.18em] text-[var(--muted)]">Alerts</p><h1 className="mt-3 text-5xl font-semibold tracking-[-.06em]">Notifications</h1><p className="mt-4 max-w-2xl text-[var(--muted)]">Changes to the things you follow, ranked by how much you said you care — delivered here, or as a push notification the moment they land.</p><div className="mt-8"><PushSubscription vapidPublicKey={vapid?.publicKey ?? null}/></div><div className="mt-6"><NotificationInbox initial={rows}/></div></main>}
