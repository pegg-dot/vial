...
export async function POST(request: NextRequest) {
  try {
    ...
    const signature = request.headers.get("stripe-signature") ?? request.headers.get("x-vialgrade-signature") ?? undefined;
    const environment = getEnvironment();
    const secret = environment.VIALGRADE_PAYMENT_PROVIDER === "stripe" ? environment.STRIPE_WEBHOOK_SECRET : process.env.VIALGRADE_MOCK_WEBHOOK_SECRET;
