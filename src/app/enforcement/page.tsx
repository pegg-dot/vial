...
export default async function Page() {
  ...
  return <div>
    <section className="border-b border-black/[.06]"><div className="mx-auto max-w-[1320px] px-5 py-16 sm:px-8 sm:py-24">
      <p className="text-[11px] font-semibold uppercase tracking-[.18em] text-rose-700">Enforcement record</p>
    ...
    <section className="mx-auto max-w-[1320px] px-5 py-16 sm:px-8 sm:py-20">
      <div className="space-y-3">{actions.map((a) => {
        ...
        return <div key={a.id} className={`rounded-[24px] border p-6 ${severe ? "border-rose-200 bg-rose-50/60" : "border-black/[.07] bg-white"}`}>
          <div className="flex flex-wrap items-start justify-between gap-3">
            <div className="min-w-0">
              <p className="text-[11px] font-semibold uppercase tracking-wide text-black/45">{AGENCY[a.agency] ?? a.agency} · {TYPE[a.action_type] ?? a.action_type}{a.outcome ? ` · ${a.outcome.replaceAll("_", " ")}` : ""}{a.action_date ? ` · ${a.action_date}` : ""}</p>
              <h2 className="mt-1 text-xl font-semibold tracking-[-.02em]">{a.title}</h2>
              <p className="mt-1 text-sm text-black/50">Named: <span className="font-medium text-black/70">{a.subject_name}</span>{a.vendor_name ? <> · matched to <Link href={`/vendors/${a.vendor_slug}`} className="font-semibold text-rose-700 hover:underline">{a.vendor_name}</Link></> : <span className="text-black/40"> · not matched to a tracked vendor</span>}</p>
              <p className="mt-2 text-sm leading-6 text-black/65">{a.summary}</p>
            </div>
            <span className={`shrink-0 rounded-full px-2.5 py-1 text-[10px] font-bold uppercase text-white ${severe ? "bg-rose-600" : a.severity === "caution" ? "bg-amber-500" : "bg-black/40"}`}>{a.severity}</span>
          </div>
          <a href={a.source_url} target="_blank" rel="noopener noreferrer" className="mt-3 inline-flex items-center gap-1 text-xs font-semibold text-rose-700 underline underline-offset-2">Read the official record <ExternalLink className="size-3" /></a>
        </div>;
      })}</div>
      {actions.length === 0 && <p className="text-sm text-black/50">No enforcement records ingested yet.</p>}
      <div className="mt-10 rounded-[28px] bg-[#111214] p-7 text-white"><Scale className="size-5 text-rose-300" />
