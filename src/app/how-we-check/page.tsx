...
import { ArrowDown, ArrowRight, Check, CircleAlert, CircleDashed, Database, Fingerprint, Scale, ShieldCheck } from "lucide-react";
    ...
    <>
      <section className="border-b border-black/[.06]">
        <div className="mx-auto max-w-[1120px] px-5 py-16 sm:px-8 sm:py-24">
          <p className="text-[11px] font-semibold uppercase tracking-[.18em] text-[var(--muted)]">How we check</p>
        ...
        <div className="mx-auto max-w-[1120px] px-5 py-16 sm:px-8 sm:py-24">
          <div className="grid gap-10 lg:grid-cols-[.72fr_1.28fr]">
            <div className="lg:sticky lg:top-28 lg:self-start">
              <p className="text-[11px] font-semibold uppercase tracking-[.18em] text-[var(--muted)]">The evidence ladder</p>
              <h2 className="mt-3 text-4xl font-semibold tracking-[-.05em]">How much a lab test really proves</h2>
      ...
      <section className="mx-auto max-w-[1120px] px-5 py-16 sm:px-8 sm:py-24">
        <div className="max-w-3xl">
          <p className="text-[11px] font-semibold uppercase tracking-[.18em] text-[var(--muted)]">Reading a lab report</p>
          <h2 className="mt-3 text-4xl font-semibold tracking-[-.05em] sm:text-5xl">One report answers six separate questions</h2>
          <p className="mt-5 text-base leading-7 text-[var(--muted)]">A polished-looking COA can still dodge most of these. On every listing, VIAL answers each one separately — established, partial, or unknown.</p>
        </div>
        <div className="mt-10 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {reportQuestions.map((q) => (
            <div key={q.title} className="rounded-[24px] border border-black/[.07] bg-white p-6">
              <h3 className="text-lg font-semibold tracking-[-.025em]">{q.title}</h3>
              <p className="mt-2 text-sm leading-6 text-[var(--muted)]">{q.detail}</p>
            </div>
          ))}
        </div>
      </section>
      ...
      <section id="limitations" className="border-y border-black/[.06] bg-white/60">
        <div className="mx-auto max-w-[1120px] px-5 py-16 sm:px-8 sm:py-24">
          <div className="max-w-3xl">
            ...
            <h2 className="mt-3 text-4xl font-semibold tracking-[-.05em] sm:text-5xl">What we refuse to imply</h2>
          </div>
      ...
      <section id="governance" className="mx-auto max-w-[1120px] px-5 py-16 sm:px-8 sm:py-24">
        <div className="grid gap-10 lg:grid-cols-[1fr_1fr] lg:items-start">
          <div>
            ...
            <h2 className="mt-3 text-4xl font-semibold tracking-[-.05em] sm:text-5xl">Software proposes. People publish.</h2>
      ...
      <section className="border-t border-black/[.06] bg-white/60">
        <div className="mx-auto max-w-[1120px] px-5 py-16 sm:px-8 sm:py-20">
          <p className="text-[11px] font-semibold uppercase tracking-[.18em] text-[var(--muted)]">Go deeper</p>
          <h2 className="mt-3 text-3xl font-semibold tracking-[-.045em]">The full records, if you want them</h2>
...
function DeepLink({ href, title, detail }: { href: string; title: string; detail: string }) {
  return (
    <Link href={href} className="group rounded-[24px] border border-black/[.07] bg-white p-6 transition hover:-translate-y-0.5 hover:border-black/[.14] hover:shadow-[0_16px_50px_rgba(20,22,27,.07)]">
