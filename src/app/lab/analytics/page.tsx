...
export default async function LabAnalyticsPage() {
  ...
  if (!context) {
    return <main className="mx-auto max-w-5xl px-5 py-14"><h1 className="text-3xl font-semibold">Laboratory analytics</h1><p className="mt-4 text-[var(--muted)]">No laboratory is associated with this account.</p></main>;
  }
  ...
  return (
    <main className="mx-auto max-w-6xl px-5 py-12">
      <p className="text-[11px] font-semibold uppercase tracking-[.18em] text-[var(--muted)]">Laboratory operations</p>
      <h1 className="mt-2 text-4xl font-semibold tracking-[-.05em] sm:text-5xl">{String(lab.display_name ?? "Laboratory")} analytics</h1>
      ...
      <section className="mt-6 overflow-hidden rounded-[28px] border border-black/[.07] bg-white">
        <div className="border-b border-black/[.06] p-6">
          <h2 className="text-xl font-semibold tracking-[-.03em]">Test order status mix</h2>
          <p className="mt-2 text-xs text-[var(--muted)]">Where your accessioned work currently sits in the pipeline.</p>
        </div>
          ...
          {orderStatus.map((row) => {
            ...
            return (
              <div key={row.status} className="flex items-center gap-4 px-6 py-4">
                ...
                <div className="h-2 flex-1 overflow-hidden rounded-full bg-black/[.05]"><div className="h-full rounded-full bg-cyan-600" style={{ width: `${Math.max(pct, 3)}%` }} /></div>
...
function Metric({ icon: Icon, value, label, sub }: { icon: React.ComponentType<{ className?: string }>; value: string; label: string; sub: string }) {
  return <div className="rounded-[22px] border border-black/[.07] bg-white p-5"><Icon className="size-4 text-black/30" /><p className="mt-5 text-3xl font-semibold tracking-[-.055em] tabular-nums">{value}</p><p className="mt-1 text-xs font-semibold">{label}</p><p className="mt-0.5 text-[11px] text-[var(--muted)]">{sub}</p></div>;
}
