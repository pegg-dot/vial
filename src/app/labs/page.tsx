  ...
  return <div>
    ...
    <section className="mx-auto max-w-[1320px] px-5 py-16 sm:px-8 sm:py-20">
      <div className="grid gap-5 lg:grid-cols-2">{labs.map(({ profile: l, usage }) => { const tier = INDEP[l.independence]; return (
        <Link key={l.slug} href={`/labs/${l.slug}`} className="group ink hard press rounded-[20px] bg-white p-7">
          ...
          <div className="mt-6 grid grid-cols-3 gap-3">
            <div className="ink-1 rounded-[12px] bg-[#f7f7f4] p-4"><p className="text-[10px] uppercase text-[var(--muted)]">Certificates</p><p className="mt-1 text-xl font-extrabold tabular-nums">{usage.coaCount}</p></div>
            <div className="ink-1 rounded-[12px] bg-[#f7f7f4] p-4"><p className="text-[10px] uppercase text-[var(--muted)]">Vendors</p><p className="mt-1 text-xl font-extrabold tabular-nums">{usage.vendorCount}</p></div>
