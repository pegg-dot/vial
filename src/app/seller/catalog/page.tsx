  ...
  return <>
    ...
    <div className="mt-7 grid gap-5 lg:grid-cols-2 xl:grid-cols-3">
      {context.products.map((row) => { const product = row as Record<string, unknown>; return <Panel key={String(product.id)} action={<StatusPill status={String(product.status)} />}>
        <div className="flex items-start gap-4"><div className="grid size-12 shrink-0 place-items-center rounded-2xl bg-[#f0efeb]"><Boxes className="size-5" /></div><div className="min-w-0"><h2 className="text-lg font-semibold tracking-[-.03em]">{String(product.title)}</h2><p className="mt-1 text-xs text-black/45">{String(product.sku || "No SKU")} · {String(product.quantity_label)}</p></div></div>
        <div className="mt-6 grid grid-cols-3 gap-3 border-t border-black/[.06] pt-5"><div><p className="text-[10px] uppercase tracking-wide text-black/35">Compound</p><p className="mt-1 truncate text-sm font-medium">{String(product.compound_name || "Unmatched")}</p></div><div><p className="text-[10px] uppercase tracking-wide text-black/35">Price</p><p className="mt-1 text-sm font-medium">${Number(product.price).toFixed(2)}</p></div><div><p className="text-[10px] uppercase tracking-wide text-black/35">Inventory</p><p className="mt-1 text-sm font-medium">{String(product.inventory)}</p></div></div>
        <div className="mt-4 flex items-center justify-between text-xs"><span className="text-black/40">Match confidence</span><b>{Math.round(Number(product.match_confidence ?? 0) * 100)}%</b></div><div className="mt-2 h-1.5 rounded-full bg-black/[.06]"><div className="h-full rounded-full bg-violet-600" style={{ width: `${Math.round(Number(product.match_confidence ?? 0) * 100)}%` }} /></div>
      </Panel>; })}
    </div>
    <Panel id="new-product" title="Create a product manually" description="Manual products stay in draft until catalog, evidence, and policy checks are complete." className="mt-7">
      <form action={createProductAction} className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
        ...
        <label className="text-sm font-medium">Price<div className="relative mt-2"><CircleDollarSign className="absolute left-3 top-1/2 size-4 -translate-y-1/2 text-black/30" /><input className="field pl-10" name="price" type="number" step="0.01" min="0" required /></div></label>
