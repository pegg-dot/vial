    ...
    <div className="mt-6 grid gap-6 xl:grid-cols-[1.1fr_.9fr]">
      ...
      <div className="space-y-6">
        <Panel title="Current interventions" description="The system explains exactly what needs attention.">
          <div className="space-y-3">{[...blockers, ...warnings].length ? [...blockers, ...warnings].map((message, index) => <div key={`${message}-${index}`} className="flex gap-3 rounded-2xl border border-amber-100 bg-amber-50 p-4"><CircleAlert className="mt-0.5 size-4 shrink-0 text-amber-700" /><p className="text-sm leading-5 text-amber-950/75">{message}</p></div>) : <div className="flex gap-3 rounded-2xl border border-emerald-100 bg-emerald-50 p-4"><ShieldCheck className="size-4 text-emerald-700" /><p className="text-sm text-emerald-900">No current readiness blockers.</p></div>}</div>
        </Panel>
        <Panel title="Operational standing">
          {[{ label: "Catalog sync", status: context.integrations.some((row) => ["connected", "sandbox_ready"].includes(String((row as Record<string, unknown>).status))) ? "ready" : "attention", Icon: Activity }, { label: "Order operations", status: context.orders.length ? "active" : "no activity", Icon: PackageCheck }].map(({ label, status, Icon }) => <div key={String(label)} className="flex items-center gap-3 border-b border-black/[.055] py-4 last:border-0"><Icon className="size-4" /><span className="flex-1 text-sm font-medium">{String(label)}</span><StatusPill status={String(status)} /></div>)}
        </Panel>
      </div>
    </div>
  </>;
