...
export default async function SellerOverviewPage() {
  ...
  return <>
    ...
    {blockers.length > 0 && <div className="mt-6 flex gap-3 rounded-[24px] border border-amber-200 bg-amber-50 p-5 text-amber-950"><CircleAlert className="mt-0.5 size-5 shrink-0" /><div><p className="font-semibold">Your next best action</p><p className="mt-1 text-sm leading-6">{blockers[0]}</p></div></div>}
      ...
      <div className="space-y-6">
        <Panel title="Self-serve connections" description="Import from an existing store, a CSV, or a public catalog scan.">
          <div className="space-y-3">
            {context.integrations.slice(0, 4).map((row) => { const integration = row as { id: string; display_name: string; provider: string; status: string; last_synced_at?: string }; return <div key={integration.id} className="flex items-center gap-3 rounded-2xl border border-black/[.06] bg-[#fafaf7] p-4"><div className="grid size-10 place-items-center rounded-xl bg-white shadow-sm"><PlugZap className="size-4" /></div><div className="min-w-0 flex-1"><p className="font-medium">{integration.display_name}</p><p className="truncate text-xs text-black/40">{integration.provider.replaceAll("_", " ")}</p></div><StatusPill status={integration.status} /></div>; })}
          </div>
        ...
        <Panel title="AI-assisted matching" description="VIAL proposes canonical compound matches, quantities, and evidence relationships. You approve every change.">
          <div className="rounded-2xl bg-gradient-to-br from-violet-600 to-[#3423bf] p-5 text-white"><Sparkles className="size-5" /><p className="mt-8 text-2xl font-semibold tracking-[-.04em]">{context.products.filter((row) => Number((row as { match_confidence?: unknown }).match_confidence ?? 0) >= .9).length} high-confidence matches</p><p className="mt-2 text-sm leading-6 text-white/60">No listing is published automatically. Imports remain reviewable drafts.</p></div>
        </Panel>
      </div>
    </div>
  </>;
