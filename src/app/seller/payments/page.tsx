...
export default async function SellerPaymentsPage() {
  ...
  return <>
      ...
      <Panel title="Connected payment account" action={<StatusPill status={String(account?.underwriting_status ?? "not_submitted")} />}>
        {account ? <div className="space-y-5">
          <div className="grid gap-3 sm:grid-cols-2">
            {[ ["Provider", account.provider], ["Account", account.provider_account_id], ["Merchant model", account.merchant_of_record], ["Configuration", account.account_configuration], ["Payouts", account.payouts_enabled ? "enabled" : "blocked"], ["Transfers", account.transfers_enabled ? "enabled" : "blocked"] ].map(([label, value]) => <div key={String(label)} className="rounded-2xl bg-black/[.035] p-4"><p className="text-[10px] font-bold uppercase tracking-[.14em] text-black/35">{String(label)}</p><p className="mt-2 break-all text-sm font-semibold">{String(value ?? "—")}</p></div>)}
          </div>
          {due.length > 0 ? <div className="rounded-2xl border border-amber-200 bg-amber-50 p-4 text-sm text-amber-900"><div className="flex items-center gap-2 font-semibold"><AlertTriangle className="size-4" />Outstanding requirements</div><ul className="mt-2 list-disc space-y-1 pl-5 text-xs">{due.map((item) => <li key={item}>{item}</li>)}</ul></div> : <div className="flex items-center gap-2 rounded-2xl border border-emerald-200 bg-emerald-50 p-4 text-sm font-medium text-emerald-800"><CheckCircle2 className="size-4" />No currently due provider requirements.</div>}
        ...
        </div> : <div className="rounded-2xl border border-dashed p-6 text-center"><CircleDollarSign className="mx-auto size-7 text-black/30" /><p className="mt-3 font-semibold">No processor account</p><p className="mt-2 text-sm text-black/45">Create the sandbox or test account boundary before beginning hosted onboarding.</p><form action={createPaymentAccountAction} className="mt-5"><button className={primaryButton}>Create payment account</button></form></div>}
      </Panel>

      <Panel title="Underwriting package" action={<StatusPill status={String(latestReview?.status ?? "not_submitted")} />}>
        <div className="rounded-2xl bg-violet-50 p-4 text-sm text-violet-950"><div className="flex items-center gap-2 font-semibold"><ShieldCheck className="size-4" />Approval is seller and catalog specific</div><p className="mt-2 text-xs leading-5 text-violet-800">Payment capabilities alone do not activate listings. VIAL also evaluates legal review, evidence, jurisdiction, customer type, seller standing, and the merchant-of-record model.</p></div>
        <form action={requestUnderwritingAction} className="mt-5 space-y-4">
          <label className="block text-sm font-medium">Requested jurisdictions<input name="jurisdictions" defaultValue="US-SANDBOX" className="mt-2 w-full rounded-xl border bg-white px-3 py-2.5 text-sm" /></label>
          <label className="block text-sm font-medium">Notes<textarea name="notes" rows={3} placeholder="Describe the catalog and operating model for review." className="mt-2 w-full rounded-xl border bg-white px-3 py-2.5 text-sm" /></label>
    ...
    <Panel title="Recent activation decisions" className="mt-6">
      <div className="space-y-3">{(workspace.decisions as Array<Record<string, unknown>>).slice(0, 8).map((decision) => <div key={String(decision.id)} className="flex flex-wrap items-center justify-between gap-3 rounded-2xl border p-4"><div><p className="font-medium capitalize">{String(decision.subject_type)} decision</p><p className="mt-1 text-xs text-black/40">{String(decision.policy_version)} · {new Date(String(decision.created_at)).toLocaleString()}</p></div><StatusPill status={String(decision.decision)} /></div>)}{workspace.decisions.length === 0 && <p className="text-sm text-black/45">No checkout activation decisions have been recorded.</p>}</div>
    </Panel>
  </>;
}
