...
export default async function SellerSupportPage() {
  ...
  return <><SellerPageHeader title="Support" description="Order, customer, evidence, and marketplace cases in one seller-scoped queue." />
    <div className="mt-7 space-y-4">{cases.length ? cases.map((item) => <Panel key={String(item.id)} action={<StatusPill status={String(item.status)} />}><div className="flex items-start gap-4"><div className="grid size-11 place-items-center rounded-2xl bg-blue-50 text-blue-700"><Headphones className="size-5" /></div><div className="flex-1"><div className="flex flex-wrap items-center gap-2"><h2 className="font-semibold">{String(item.subject)}</h2><StatusPill status={String(item.priority)} /></div><p className="mt-1 text-sm text-black/45">{String(item.category).replaceAll("_", " ")} · {String(item.requester_label)}</p><p className="mt-3 flex items-center gap-1.5 text-xs text-black/40"><Timer className="size-3.5" />SLA {String(item.sla_due_at || "Not assigned")}</p></div></div></Panel>) : <Panel><p className="text-sm text-black/45">No open support cases.</p></Panel>}</div>
  </>;
}
