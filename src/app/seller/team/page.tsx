  ...
  return <>
    ...
    <div className="mt-7 grid gap-6 xl:grid-cols-[1.15fr_.85fr]">
      <Panel title="Members" description={`${context.team.length} seller team records`}>
        <div className="space-y-3">{context.team.map((row) => { const member = row as Record<string, unknown>; return <div key={String(member.id)} className="ink-1 flex items-center gap-4 rounded-[14px] bg-[#fafaf7] p-4"><div className="ink-1 grid size-11 place-items-center rounded-[12px] bg-white"><UsersRound className="size-4" /></div><div className="min-w-0 flex-1"><p className="truncate font-bold">{String(member.display_name)}</p><p className="truncate text-xs font-medium text-[var(--muted)]">{String(member.email)} · {String(member.role).replaceAll("_", " ")}</p></div><StatusPill status={String(member.status)} /></div>; })}</div>
      </Panel>
