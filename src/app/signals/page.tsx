...
import { publicSignalLabel } from "@/server/intelligence/signal-kinds";
...
export default async function SignalsPage() {
  ...
  return <section className="mx-auto max-w-[1320px] px-5 py-14 sm:px-8 sm:py-20">
    ...
    <div className="mt-12 grid gap-5 md:grid-cols-2 xl:grid-cols-3">{signals.map(signal=>{const target=href(signal.entityType,signal.entitySlug);return <article key={signal.id} className="group ink-1 hard press rounded-[18px] bg-white p-6"><div className="flex items-start justify-between gap-4"><span className="ink-1 grid size-11 place-items-center rounded-2xl bg-[#e6fbf4]"><Sparkles className="size-4 text-[#0e8f80]"/></span><div className="text-right"><p className="text-3xl font-extrabold tracking-[-.055em] tabular-nums">{Math.round(signal.score)}</p><p className="text-[9px] uppercase tracking-[.12em] text-[var(--muted)]">how big it is</p></div></div><p className="mt-7 text-[10px] font-bold uppercase tracking-[.15em] text-[#0e8f80]">{publicSignalLabel(signal.signalType)}</p><h2 className="mt-3 text-xl font-extrabold tracking-[-.03em]">{signal.title}</h2><p className="mt-3 text-sm font-medium leading-6 text-[var(--muted)]">{signal.summary}</p><div className="mt-6 flex items-center justify-between border-t border-[#111214]/12 pt-4"><div><p className="text-xs font-bold">{signal.entityLabel}</p><p className="mt-1 text-[10px] font-medium text-[var(--muted)]">we&rsquo;re {Math.round(signal.confidence*100)}% sure of the data</p></div>{target&&<Link href={target} className="ink-1 grid size-9 place-items-center rounded-full transition group-hover:bg-[#111214] group-hover:text-white" aria-label={`Open ${signal.entityLabel}`}><ArrowUpRight className="size-3.5"/></Link>}</div></article>})}</div>
