export function PriceSparkline({ values, accent = "#6d5dfc", height = 58 }: { values: number[]; accent?: string; height?: number }) {
  const width = 180;
  const padding = 5;
  ...
  return (
    <svg viewBox={`0 0 ${width} ${height}`} className="h-full w-full" role="img" aria-label={`Price history from $${values[0]} to $${values.at(-1)}`}>
      <defs>
        <linearGradient id={`gradient-${accent.replace("#", "")}-${height}`} x1="0" y1="0" x2="0" y2="1">
          <stop offset="0" stopColor={accent} stopOpacity="0.25" />
          <stop offset="1" stopColor={accent} stopOpacity="0" />
        </linearGradient>
      </defs>
      <polygon points={areaPoints} fill={`url(#gradient-${accent.replace("#", "")}-${height})`} />
