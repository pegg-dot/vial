...
export function SearchTrigger({ className = "" }: { className?: string }) {
  return <Link href="/verify" className={`ink hard press group flex w-full items-center gap-3 rounded-2xl bg-white px-5 py-4 text-left ${className}`}>
    ...
    {/* Visible on every screen: on a phone this was `hidden sm:block`, so the primary call to
        action had no button at all on the device most buyers arrive with. */}
    <span className="shrink-0 rounded-xl bg-[#2b31d8] px-4 py-2 text-xs font-bold text-white">Check it</span>
  </Link>;
}
