      ...
      <div className="mx-auto flex h-[72px] max-w-[1320px] items-center gap-6 px-5 sm:px-8">
        ...
        <div className="ml-auto flex items-center gap-2">
          ...
          >
            ...
            <span className="flex-1">Search market</span>
          ...
          <Link href="/cart" className="relative hidden rounded-full border border-black/[.08] bg-white/70 px-4 py-2.5 text-sm font-semibold transition hover:bg-white sm:block">
            Cart
            {cart?.lines?.length ? <span className="absolute -right-1 -top-1 grid size-5 place-items-center rounded-full bg-violet-600 text-[10px] text-white">{cart.lines.reduce((total, line) => total + line.quantity, 0)}</span> : null}
          </Link>
          <Link
            href="/watchlist"
      ...
      {mobileOpen && (
        <nav aria-label="Mobile navigation" className="border-t border-black/[.06] bg-[var(--background)] px-5 py-4 md:hidden">
          <div className="mx-auto grid max-w-[1320px] gap-1">
            {[...nav, { href: "/compare", label: "Compare" }, { href: "/saved-searches", label: "Saved searches" }, { href: "/updates", label: "Updates" }, { href: "/cart", label: "Cart" }, { href: "/methodology", label: "Methodology" }, { href: "/operations", label: "Operations" }, { href: "/watchlist", label: "Watchlist" }, { href: "/account", label: "Account" }].map((item) => (
              <Link
                key={item.href}
                href={item.href}
