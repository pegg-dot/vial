...
export function Wordmark({ inverse = false }: { inverse?: boolean }) {
  return (
    <Link href="/" aria-label="VIAL home" className="inline-flex items-center gap-2.5">
      <span
      ...
      <span className={`text-[15px] font-black tracking-[0.18em] ${inverse ? "text-white" : "text-[#111214]"}`}>VIAL</span>
    </Link>
  );
}
