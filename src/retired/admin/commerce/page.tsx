...
export default async function AdminCommercePage() {
  ...
  return <div>
    <div className="flex flex-wrap items-end justify-between gap-4"><div><p className="text-xs font-extrabold uppercase tracking-[.18em] text-[#2b31d8]">Commerce control plane</p><h1 className="mt-2 text-4xl font-extrabold tracking-[-.05em]">Approved commerce architecture</h1><p className="mt-3 max-w-3xl text-sm font-medium leading-6 text-[var(--muted)]">VialGrade separates processor capability, seller underwriting, SKU approval, customer eligibility, jurisdiction, tax liability, fraud, merchant model, settlement, and activation.</p></div><span className={`ink-1 rounded-full px-4 py-2 text-sm font-extrabold ${approved.mode === "live" ? "bg-[#fff1f0] text-[#d3372c]" : "bg-[#e9eaff] text-[#2b31d8]"}`}>{String(approved.provider)} · {String(approved.mode)}</span></div>
