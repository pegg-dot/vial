...
/**
 * Ordered by when the search was SAVED, not when its row was last written.
 *
 * `updated_at DESC` made the list re-sort itself behind the reader's back: every alert-mode patch
 * and every run bumps `updated_at`, so a row the reader had just touched in place silently jumped
 * to the top on the next load. The client maps updates in place and prepends new saves, and
 * `created_at DESC` is exactly that order — so what the reader sees and what a reload gives them
 * agree, and nothing moves under the cursor mid-interaction. `id` breaks ties (a seed inserts many
 * rows inside one transaction, where NOW() is a single timestamp) so the order is deterministic.
 */
export async function listSavedSearches(userId:string,connection?:SqlConnection){const db=connection??await getDatabase();const result=await db.query<SavedSearchRow>(`SELECT * FROM saved_searches WHERE user_id=$1 ORDER BY created_at DESC,id DESC`,[userId]);return result.rows.map(toSavedSearch)}
...
/**
 * `deliverAfter` is REQUIRED, and deliberately so.
 *
 * This used to fall back to a private `quietDeliveryTime` — a second, digest-blind implementation
 * of the quiet-hours rule, reachable whenever a caller omitted the argument. Every caller passed it,
 * which is what made it a trap rather than a bug: the divergence was waiting for the next one. The
 * policy in server/notifications/policy.ts is the only thing entitled to decide when a notification
 * becomes visible, and now the type system says so.
 */
export async function upsertUserNotification(userId:string,input:{category:string;title:string;body:string;actionHref?:string;relevanceScore:number;dedupeKey:string;deliverAfter:Date},connection?:SqlConnection):Promise<{inserted:boolean;alreadyPushed:boolean}>{const db=connection??await getDatabase();const deliverAfter=input.deliverAfter;const result=await db.query<{inserted:boolean;pushed_at:Date|string|null}>(`INSERT INTO user_notifications(id,user_id,category,title,body,action_href,relevance_score,dedupe_key,deliver_after) VALUES($1,$2,$3,$4,$5,$6,$7,$8,$9) ON CONFLICT(user_id,dedupe_key) DO UPDATE SET title=EXCLUDED.title,body=EXCLUDED.body,action_href=EXCLUDED.action_href,relevance_score=GREATEST(user_notifications.relevance_score,EXCLUDED.relevance_score),deliver_after=LEAST(user_notifications.deliver_after,EXCLUDED.deliver_after) RETURNING (xmax = 0) AS inserted, pushed_at`,[newId("notification"),userId,input.category,input.title,input.body,input.actionHref??null,input.relevanceScore,input.dedupeKey,deliverAfter]);const row=result.rows[0];return{inserted:Boolean(row?.inserted),alreadyPushed:Boolean(row?.pushed_at)}}

/**
 * Records that a push for this notification actually went out.
 *
 * Written only after the send is attempted, so a crash between the two re-tries the push next tick
 * rather than swallowing it. Scoped by user_id: a dedupe key is unique per reader, never globally.
 */
export async function markNotificationPushed(userId:string,dedupeKey:string,connection?:SqlConnection){const db=connection??await getDatabase();await db.query(`UPDATE user_notifications SET pushed_at=NOW() WHERE user_id=$1 AND dedupe_key=$2 AND pushed_at IS NULL`,[userId,dedupeKey])}
