...
export async function seedConsumerIntelligence(db:SqlConnection){
  ...
  await upsertUserNotification(userId,{category:"evidence_update",title:"Northstar report confirmation remains current",body:"The latest reviewed record still shows issuer confirmation and batch linkage.",actionHref:"/products/northstar-bpc-157-10mg",relevanceScore:0.92,dedupeKey:"seed:northstar-evidence"},db);
  await upsertUserNotification(userId,{category:"price_change",title:"Helix BPC-157 moved inside your price range",body:"The fictional observed price is now within your configured $25–$95 range.",actionHref:"/products/helix-bpc-157-10mg",relevanceScore:0.78,dedupeKey:"seed:helix-price"},db);
