...
export async function listPublicPassports() { await ensureEvidenceNetworkSeed(); const db = await getDatabase(); return (await db.query(`SELECT bp.*,o.display_name vendor_name,p.name product_name,l.slug listing_slug,(SELECT COUNT(*) FROM passport_evidence_links pel WHERE pel.passport_id=bp.id AND pel.status='active') evidence_links,(SELECT COUNT(*) FROM evidence_conflicts ec WHERE ec.passport_id=bp.id AND ec.status='open') open_conflicts FROM batch_passports bp LEFT JOIN organizations o ON o.id=bp.vendor_id LEFT JOIN products p ON p.id=bp.product_id LEFT JOIN listings l ON l.id=bp.listing_id WHERE bp.status='published' ORDER BY bp.updated_at DESC`)).rows; }
