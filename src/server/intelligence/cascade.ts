...
export async function runPublicationCascade(tx: SqlConnection, input: {
  ...
  if (compound.priceSpreadPct >= 20) {
    await upsertOpportunity(tx, {
      ...
      entityType: "compound",
      entityId: context.compound_id,
      title: `${context.compound_name} prices are fragmented`,
      summary: `The public listing range spans ${compound.priceSpreadPct.toFixed(1)}% around the median, creating a strong comparison and normalization opportunity.`,
  ...
  if (compound.availableListings <= 1) {
    await upsertOpportunity(tx, {
      ...
      entityType: "compound",
      entityId: context.compound_id,
      title: `${context.compound_name} has thin visible availability`,
      summary: `Only ${compound.availableListings} active listing is currently visible as available, so availability alerts and source expansion have high user value.`,
      score: 76,
      confidence: 0.92,
      evidence: compound,
    });
  ...
  if (compound.documentationCoverage < 70) {
    await upsertOpportunity(tx, {
      ...
      entityType: "compound",
      entityId: context.compound_id,
      title: `${context.compound_name} has incomplete public evidence coverage`,
      summary: `Only ${compound.documentationCoverage}% of observed listings expose a dated report. A normalized evidence view can materially reduce buyer research friction.`,
  ...
  if (vendor.documentationCurrent < vendor.productCount) {
    await upsertOpportunity(tx, {
      ...
      entityType: "vendor",
      entityId: context.vendor_id,
      title: `${context.vendor_name} has catalog evidence gaps`,
      summary: `${vendor.documentationCurrent} of ${vendor.productCount} observed products have a dated public report.`,
  ...
  if (context.vendor_profile_status === "unclaimed") {
    await upsertOpportunity(tx, {
      ...
      entityType: "vendor",
      entityId: context.vendor_id,
      title: `${context.vendor_name} can claim its market record`,
      summary: "This independent vendor profile has active catalog observations but no verified operator controlling corrections or direct feeds.",
      score: 68,
      confidence: 0.98,
  ...
  if (!input.after.reportDate || input.after.reportDate === "Not located") {
    await upsertOpportunity(tx, {
      ...
      entityType: "listing",
      entityId: input.listingId,
      title: `${context.vendor_name} ${context.product_name} needs a current evidence record`,
      summary: "The listing changed, but no dated public report is connected to its current state.",
      score: 81,
      confidence: 0.97,
  ...
  if (input.predicate === "batchCode" && !input.after.reportConfirmed) {
    await upsertOpportunity(tx, {
      ...
      entityType: "listing",
      entityId: input.listingId,
      title: `New ${context.product_name} batch needs corroborating evidence`,
      summary: `Batch ${input.after.batchCode} is now visible, but issuer confirmation has not been established.`,
      score: 88,
      confidence: 0.98,
  ...
  if (compound.medianPrice > 0) {
    ...
    if (deviation >= 0.2) {
      await upsertOpportunity(tx, {
        ...
        entityType: "listing",
        entityId: input.listingId,
        title: `${context.vendor_name} is a visible price outlier`,
        summary: `The listing is ${(deviation * 100).toFixed(1)}% away from the current ${context.compound_name} median, increasing the value of transparent comparisons.`,
