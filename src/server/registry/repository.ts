...
// The public identity namespaces VIAL mints stable IDs in. These are the category
// standard others cite — `vial:compound:bpc-157`, `vial:lab:aperture-analytical`.
...
export interface RegistryRecord {
  ...
  relationships: { relation: string; vialId: string | null; displayName: string; entityType: string }[];
}
...
// Idempotent minter. A VIAL ID is keyed on the stable *source identity*
...
export async function mintVialId(db: SqlConnection, input: MintInput): Promise<{ vialId: string; minted: boolean }> {
  ...
  if (existing) {
    if (existing.canonical_slug !== input.slug) {
      ...
      await db.query(
        `INSERT INTO registry_identifier_aliases(id,vial_id,alias,normalized_alias,alias_type)
         VALUES($1,$2,$3,$4,'former-slug') ON CONFLICT(vial_id,normalized_alias) DO NOTHING`,
        [newId("regalias"), existing.vial_id, existing.canonical_slug, normalizeTerm(existing.canonical_slug)],
      );
    }
    await db.query(
      `UPDATE registry_identifiers SET display_name=$2,canonical_slug=$3,provenance_url=$4,
       ...
       WHERE vial_id=$1`,
      [existing.vial_id, input.displayName, input.slug, provenanceUrl, input.currentEntityId ?? null, attributes],
    );
...
// Resolves a public VIAL ID to its full record, following a redirect (merge) one hop.
export async function getRegistryRecord(vialId: string, connection?: SqlConnection): Promise<RegistryRecord | null> {
  const db = connection ?? (await getDatabase());
  const row = (await db.query<RegistryRow>(`SELECT * FROM registry_identifiers WHERE vial_id=$1`, [vialId])).rows[0];
  if (!row) return null;
  if (row.status === "redirected" && row.redirects_to && row.redirects_to !== vialId) {
    return getRegistryRecord(row.redirects_to, db);
  }
  const aliases = (await db.query<{ alias: string }>(`SELECT alias FROM registry_identifier_aliases WHERE vial_id=$1 ORDER BY created_at`, [vialId])).rows.map(r => r.alias);
  const relationships = row.current_entity_id
    ? (await db.query<{ relation_type: string; to_entity_id: string; display_name: string; entity_type: string; vial_id: string | null }>(
        `SELECT r.relation_type,r.to_entity_id,e.display_name,e.entity_type,ri.vial_id
      ...
      )).rows.map(r => ({ relation: r.relation_type, vialId: r.vial_id, displayName: r.display_name, entityType: r.entity_type }))
    : [];
  return {
    vialId: row.vial_id,
...
export interface RegistryResolution {
  best: { vialId: string; displayName: string; entityType: RegistryEntityType; score: number; matchedOn: string } | null;
  candidates: { vialId: string; displayName: string; entityType: RegistryEntityType; score: number }[];
}

// Maps a messy real-world label (a vendor's product string, a former slug, a typo)
// onto a canonical VIAL ID — the flywheel that turns every observation into resolution.
export async function resolveToRegistry(label: string, type?: RegistryEntityType, connection?: SqlConnection): Promise<RegistryResolution> {
  ...
  if (direct) return { best: { vialId: direct.vial_id, displayName: direct.display_name, entityType: direct.entity_type, score: 1, matchedOn: direct.canonical_slug }, candidates: [] };
  ...
  // 3. Fuzzy match through the canonical entity graph, mapped back to a VIAL ID.
  ...
  for (const candidate of resolution.candidates) {
    ...
    if (!reg) continue;
    const mapped = { vialId: reg.vial_id, displayName: reg.display_name, entityType: reg.entity_type, score: candidate.score };
