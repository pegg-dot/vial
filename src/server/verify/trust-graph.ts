...
export function composeVerdict(v: VerdictInput): ComposedVerdict {
  ...
  if (severe > 0) { factors.push({ ok: false, label: "Government enforcement", detail: `On a public enforcement record — ${severe} proven-severe action${severe === 1 ? "" : "s"} (FDA/DOJ/FTC).` }); reasons.avoid.push("a proven enforcement action"); }
  else if (cautionActions > 0) { factors.push({ ok: false, label: "Government enforcement", detail: `Named in ${cautionActions} public regulatory record${cautionActions === 1 ? "" : "s"} (e.g. an FDA warning letter).` }); reasons.caution.push("a public regulatory record"); }
  ...
  if (v.coaCount > 0) {
    ...
    factors.push({ ok: true, label: "Independent testing", detail: `${bits.join(" · ")}.` });
    reasons.trust.push(`${v.coaCount} independent lab test${v.coaCount === 1 ? "" : "s"}${v.blindCount > 0 ? " (incl. blind purchases)" : ""}`);
  } else factors.push({ ok: false, label: "Independent testing", detail: "No third-party lab tests on record for this vendor." });
  ...
  if (hasRiskFlags) { factors.push({ ok: false, label: "Scam & red flags", detail: riskDim!.value }); reasons.caution.push("open risk flags in its reputation record"); }
  if (evidenceConflict) { factors.push({ ok: null, label: "Conflicting evidence", detail: `Independent evidence for this vendor conflicts with itself (${corrDim!.value}) — a genuine reason for caution.` }); reasons.caution.push("independent evidence that conflicts with itself"); }
  ...
  if (scored.length > 0) {
    ...
    factors.push({ ok: anyGood ? true : anyBad ? false : null, label: "Third-party trackers", detail: `${best} (their opinion, not ours).` });
  ...
  if (v.signals) {
    ...
    if (young) { factors.push({ ok: false, label: "Domain age", detail: v.signals.domain_age_note ?? "Recently registered domain." }); reasons.caution.push("a very young domain"); }
    if (riskCopy) { factors.push({ ok: false, label: "Storefront signal", detail: v.signals.notable_copy ?? "" }); reasons.caution.push("a risk signal on its own storefront"); }
  ...
  if (v.community) {
    ...
    if (/scam|negative|fraud/.test(c)) { factors.push({ ok: false, label: "Community", detail: "Community reports lean toward scam/quality complaints." }); reasons.avoid.push("community scam reports"); }
    else if (/vouch|positive|trust/.test(c)) { factors.push({ ok: true, label: "Community", detail: "Community mentions lean positive." }); reasons.trust.push("positive community mentions"); }
  }

  // 8. Operator-network linkage — shared identifiers with a flagged storefront.
  const scamLink = v.links.find((l) => l.strength === "strong" && findKnownVendorBySlug(l.linkedSlug)?.redFlag);
  if (scamLink) { factors.push({ ok: false, label: "Operator network", detail: `Shares hard identifiers with ${findKnownVendorBySlug(scamLink.linkedSlug)?.name ?? scamLink.linkedSlug}, a flagged storefront.` }); reasons.avoid.push("shared identifiers with a flagged storefront"); }
  ...
  if (v.status && v.status.status !== "operating" && v.status.status !== "unknown" && v.status.status !== "blocked") {
    ...
    factors.push({ ok: false, label: "Site status", detail: dead ? "Their storefront is offline or parked." : "Their storefront redirects away." });
  ...
  if (v.flagCount > 0) { factors.push({ ok: false, label: "COA integrity", detail: `${v.flagCount} certificate-integrity flag${v.flagCount === 1 ? "" : "s"} (e.g. a cert that resolves to a different maker).` }); reasons.caution.push("a certificate-integrity flag"); }
  ...
  return { verdict, headline, summary, factors, weighed };
}
