...
import { POST as postProposal } from "@/app/api/public/v1/id/[registryId]/evidence-proposals/route";

function req(token: string | null, registryId: string, body: unknown) {
  const headers: Record<string, string> = { "content-type": "application/json" };
  if (token) headers.authorization = `Bearer ${token}`;
  return new Request(`https://api.vialgrade.test/api/public/v1/id/${registryId}/evidence-proposals`, { method: "POST", headers, body: JSON.stringify(body) });
}
const params = (registryId: string) => ({ params: Promise.resolve({ registryId }) });
...
describe("VIAL 10.0 external evidence submission (proposal-only)", () => {
  ...
  it("accepts a lab-token evidence proposal against a batch VIAL ID and lands it in review — never published", async () => {
    ...
    const res = await postProposal(req(token.token, "vialgrade:batch:hx-bpc-2607", { proposalType: "evidence-link", payload: { reportNumber: "APR-2607-999", note: "corroborating third sample" } }), params("vialgrade:batch:hx-bpc-2607"));
  ...
  it("rejects an unauthenticated token (401) and a token without evidence:propose scope (403)", async () => {
    const noAuth = await postProposal(req(null, "vialgrade:batch:hx-bpc-2607", { proposalType: "evidence-link", payload: {} }), params("vialgrade:batch:hx-bpc-2607"));
    ...
    const forbidden = await postProposal(req(wrongScope.token, "vialgrade:batch:hx-bpc-2607", { proposalType: "evidence-link", payload: {} }), params("vialgrade:batch:hx-bpc-2607"));
    expect(forbidden.status).toBe(403);
  });

  it("returns 404 for an unknown VIAL ID and 400 for a disallowed proposal type", async () => {
    const token = await createLaboratoryApiToken({ laboratoryId: "lab:aperture", label: "api", scopes: ["evidence:propose"], actorId: "user:lab:elena" });
    const missing = await postProposal(req(token.token, "vialgrade:batch:ghost", { proposalType: "evidence-link", payload: {} }), params("vialgrade:batch:ghost"));
    expect(missing.status).toBe(404);

    const badType = await postProposal(req(token.token, "vialgrade:batch:hx-bpc-2607", { proposalType: "publish-now", payload: {} }), params("vialgrade:batch:hx-bpc-2607"));
