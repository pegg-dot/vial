...
describe("inbound visitor funnel", () => {
  it("counts people, not page views", async () => {
    ...
    for (const path of ["/", "/market", "/products/x"]) {
      await recordPageView({ path, ip: "1.1.1.1", userAgent: "UA" }, db);
    }
    await recordPageView({ path: "/", ip: "2.2.2.2", userAgent: "UA" }, db);
  ...
  it("attributes a source, and treats our own pages as direct", async () => {
    const db = await getDatabase();
    await recordPageView({ path: "/", referrer: "https://www.reddit.com/r/Peptides/x", ip: "1.1.1.1", userAgent: "UA" }, db);
    await recordPageView({ path: "/market", referrer: "https://vialgrade.com/", selfHost: "vialgrade.com", ip: "2.2.2.2", userAgent: "UA" }, db);
  ...
  it("computes the arrivals-to-buyers rate from a real outbound click", async () => {
    const db = await getDatabase();
    await recordPageView({ path: "/products/x", ip: "1.1.1.1", userAgent: "UA" }, db);
    await recordPageView({ path: "/products/y", ip: "3.3.3.3", userAgent: "UA" }, db);
