...
describe("versioned session envelope",()=>{
 const original=process.env.VIAL_SESSION_SECRET;const now=Date.now();
 beforeEach(()=>{process.env.VIAL_SESSION_SECRET="unit-test-session-secret-at-least-32-characters"});
